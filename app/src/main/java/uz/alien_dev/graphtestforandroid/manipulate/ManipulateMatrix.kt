...
class ManipulateMatrix : View {
    ...
    class GraphLoop(private val graph: ManipulateMatrix) : Thread() {
    ...
    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
        ...
        matrix = IntArray(surfaceWidth * surfaceHeight) { Color.BLACK }
    }

    private fun pixel(x: Int, y: Int, newColor: Int) {
        matrix[y * bitmap.width + x] = newColor
    }

    private fun createTask() {
        for (x in 0 until width) for (y in 0 until height) pixel(x, y, Color.rgb(x * 255 / width, y * 255 / height, 255))
    }

    fun update() {
        createTask()
        bitmap.setPixels(matrix, 0, bitmap.width, 0, 0, bitmap.width, bitmap.height)
    }
